    ...
    let parseMegaindex = domain =>
      new Promise((resolve, reject) => {
        try {
          ...
          page.goto(megaindex, { waitUntil: 'networkidle2' });
          console.log('Ожидаю загрузки селектора: #serp .count');
          page.waitForSelector('#serp .count');
          // await page.waitForTimeout(5000);

          let traffic = page.$eval('#serp .count', traffic_organic => {
            if (traffic_organic.textContent.includes('M')) {
              ...
              return (
                traffic_organic.textContent.replace(/[^0-9.,\s]/g, '') * 1000
              );
            } else {
          ...
          let traffic_season =
            traffic !== '' ? (dataObj['traffic'] * 0.9).toFixed(0) : '';
          ...
          let profit_await =
            traffic_season === '' ? '' : (traffic_season * 0.3).toFixed(0);
          ...
          let evaluate_min =
            profit_await === ''
              ? ''
              : (project_stage * profit_await * 0.5).toFixed(0);
          dataObj['evaluate_min'] = evaluate_min;

          let evaluate_middle =
            profit_await === ''
              ? ''
              : (project_stage * profit_await * 0.75).toFixed(0);
          dataObj['evaluate_middle'] = evaluate_middle;

          let evaluate_max =
            profit_await === ''
              ? ''
              : (project_stage * profit_await).toFixed(0);
